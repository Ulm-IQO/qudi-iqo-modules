...
class PulsedFiniteSamplingInput(FiniteSamplingInputInterface):
    ...
    _channel_mw= ConfigOption(name='channel_mw', missing='error')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._mode = 'cw'
        self._mode_old = 'cw'
        self._runs_per_period = 4
        self._t_pi = 1.0e-6
        self._t_pre = 2.0
        #self._t_delay = 5.0e-6
        self._t_delay = 2.0e-6
        self._t_readout = 5.0e-6
    ...
    def get_buffered_samples(self, number_of_samples=None):
        ...
        if self.mode in ['pulsed', 'pulsed_awg']:
            ...
            with open('C:\\Users\\yy3\\qudi\\Data\\2024\\07\\trace.npy', 'wb+') as f:
                np.save(f, data2D)
    ...
    def _get_waveform_cw(self):
        ...
        waveform = [
            {'active_channels':[aom                   ], 'length': 2.0,         'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom, mw,          next], 'length': period*0.35, 'inst':LOOP, 'inst_data':N_Loop},
            {'active_channels':[aom, mw, readout      ], 'length': period*0.6,  'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom                   ], 'length': period*0.35, 'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom,     readout      ], 'length': period*0.6,  'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom                   ], 'length': period*0.1,  'inst':END,  'inst_data':1     },
    ...
    def _get_waveform_cw_awg(self):
        ...
        waveform = [
            ...
            {'active_channels':[aom, mw,      readout      ], 'length': period*0.7, 'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom                        ], 'length': period*0.2, 'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom                        ], 'length': period*0.1, 'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom,          readout      ], 'length': period*0.7, 'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom                        ], 'length': period*0.2, 'inst':END,  'inst_data':1     },
            {'active_channels':[                           ], 'length': period*0.1, 'inst':None, 'inst_data':None  },
        ]
        return waveform
    ...
    def _get_waveform_pulsed(self):
        ...
        t_init = T/2 - t_wait - t_mw - t_delay - t_readout
        ...
        waveform = [
            ...
            {'active_channels':[aom,     readout      ], 'length': t_readout,   'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom,                  ], 'length': t_init,      'inst':CONT, 'inst_data':None  },
            
            {'active_channels':[                      ], 'length': t_wait+t_mw, 'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom,                  ], 'length': t_delay,     'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom,     readout      ], 'length': t_readout,   'inst':CONT, 'inst_data':None  },
            {'active_channels':[aom,                  ], 'length': t_init,      'inst':END,  'inst_data':2     },
    ...
    def _init_digitizer(self):
        ...
        if self.mode in ['pulsed', 'pulsed_awg']:
            record_length_s = self._t_readout
            bin_width_s = record_length_s/520
