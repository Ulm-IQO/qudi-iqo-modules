...
class NIXTTSeriesInStreamer(DataInStreamInterface):
    """
    ...
    def available_samples(self):
        ...
        if self._di_task_handles == list():
            ...
            return self.__buffer_size
        elif self._ai_task_handle is None:
    ...
    def read_data(self, number_of_samples=None):
        ...
        tt_data = self.tt_counter.getData()
        #Now add TT data into the _data_buffer
        
        data_buffer = self._data_buffer[:total_samples].reshape((self.number_of_channels,
                                                          number_of_samples))
        print(data_buffer.shape, tt_data.shape)
        if len(data_buffer) > 0:
            data_buffer = np.vstack(( data_buffer,  tt_data[:total_sample]))
        else:
            data_buffer = tt_data
