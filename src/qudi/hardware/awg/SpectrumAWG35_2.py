...
class AWG:
    ...
    def init_sequence_mode(self, step_count):
        ...
        for card in self.cards:
            card.set32(SPC_SEQMODE_MAXSEGMENTS, max_segments)
            card.set32(SPC_SEQMODE_STARTSTEP, 0)
    ...
    def upload_wfm_thread(self, wfm, wfm_size):
        ...
        t1 = threading.Thread(
            ...
            args=(self.cards[0], [wfm[0], wfm[1]], wfm_size),
        )
        t2 = threading.Thread(
            target=self._run_upload_wfm,
            args=(self.cards[1], [wfm[2], wfm[3]], wfm_size),
        )
        del wfm
        t1.start()
        t2.start()
    ...
    def upload_wfm(self, wfm, wfm_size, ):
        ...
        self._run_upload_wfm(
            self.cards[0], [wfm[0], wfm[1]], wfm_size,
        )
        self._run_upload_wfm(
            self.cards[1], [wfm[2], wfm[3]], wfm_size,
        )
    ...
    def _run_upload_wfm(self, card, data, data_size):
        if self._enable_debug:
            print('run AWG._run_upload_wfm()')

        res = card.upload(
            data_size, data=data[0], data1=data[1], block=True,
        )
        if res == 0:
            pass
        else:
    ...
    def load_sequence(self, seq, forced_trigger=False):
        if self._enable_debug: print('run AWG.load_sequence()')
        self.uploading = True
        self.stop()
        self.start_time = time.time()

        num_seq = len(seq)
        self.init_sequence_mode(num_seq)
        ...
        for i, step in enumerate(seq):
            wfm, loops = step
            seg_size = wfm[0].size
            self.upload_step(wfm, seg_size, i)
            self.write_sequence_step(i, i, loops, (i + 1) % num_seq, nextcond)
            # Set 'goto' of the last wfm to 0, so that it points back to the first wfm.

    def upload_step(self, wfm, wfm_size, step_index):
        self.uploading = True
        if self._enable_debug: print('run AWG.upload_wfm()')
        # if not data0 == None or not data1 == None:
        ...
        self.cards[0].set_current_segment(step_index)
        res = self.cards[0].upload(
            wfm_size, data=wfm[0], data1=wfm[1], is_seq_segment=True, block=True,
        )
        if res : print('Error on card 0:',str(res))

        self.cards[1].set_current_segment(step_index)
        res = self.cards[1].upload(
            wfm_size, data=wfm[2], data1=wfm[3], is_seq_segment=True, block=True,
        )
    ...
    def write_sequence_step(self, step_index, mem_segment_index, loops, goto, next_condition):
        if self._enable_debug:
            print(
                'run AWG.write_sequence_step()',
                step_index, mem_segment_index, loops, goto, next_condition,
            )
        for card in self.cards:
    ...
    def load_multi(self, seq, forced_trigger=False):
    ...
    def upload_segment(self, step, step_index, segsize):
        self.uploading = True
        if self._enable_debug: print('run AWG.upload_segment()')

        self.start_upload_time = time.time()

        res = self.cards[0].upload_segment(
            segsize, step_index, data=step[0], data1=step[1], block=True,
        )
        if res : print('Error on card 0:',str(res))

        res = self.cards[1].upload_segment(
            segsize, step_index, data=step[2], data1=step[3], block=True,
        )
        if res : print('Error on card 1:',str(res))
...
class Card():
    ...
    # @profile
    def upload(self, num_samples, data=None, data1=None,
        ...
        # with open('C:\\Users\\yy3\\qudi\\Data\\2024\\05\\2024-05-06\\card%d_dataxx.npy' % self.cardNo, 'wb+') as f:
        #     np.save(f, data[0:num_samples])
        ...
        if not is_buffered:
            mem_size = num_samples
            while (not (mem_size % 32 == 0)) or (is_seq_segment is True and num_samples < 192):
                mem_size += 1
            self.set_memory_size(mem_size, is_seq_segment=is_seq_segment)

        # set the buffer
        BufferSize = uint64(num_samples * bytes_per_sample * used_channels)
        pvBuffer = create_string_buffer(BufferSize.value)

        pnBuffer = np.zeros(num_samples * used_channels, dtype=np.int16)
        pnBuffer[0:num_samples * 2:2] = data[0:num_samples]
        pnBuffer[1:num_samples * 2:2] = data1[0:num_samples]
        ...
        # with open('C:\\Users\\yy3\\qudi\\Data\\2024\\05\\2024-05-06\\card%d_pnBuffer_new.npy' % self.cardNo, 'wb+') as f:
        #     np.save(f, pnBuffer_uint)
    ...
    def upload_segment(self, segsize, segidx, data=None, data1=None, block=False):
        ...
        data_size = data.size
        pnBuffer[0:data_size * 2:2] = data[:data_size]
        pnBuffer[1:data_size * 2:2] = data1[:data_size]
        
        with open('C:\\Users\\yy3\\qudi\\Data\\2024\\06\\2024-06-11\\card%d_data0-%d.npy' % (self.cardNo, segidx), 'wb+') as f:
            np.save(f, data)
        
        with open('C:\\Users\\yy3\\qudi\\Data\\2024\\06\\2024-06-11\\card%d_data1-%d.npy' % (self.cardNo, segidx), 'wb+') as f:
            np.save(f, data1)
        ...
        print(segidx, segidx*buffer_size)
        spcm_dwDefTransfer_i64(
            self.handle, SPCM_BUF_DATA, SPCM_DIR_PCTOCARD,
            uint32(0),
            pvBuffer,
            uint64(segidx*buffer_size),
            BufferSize
        )

        with open('C:\\Users\\yy3\\qudi\\Data\\2024\\06\\2024-06-11\\card%d_pnBuffer%d.npy' % (self.cardNo, segidx), 'wb+') as f:
            np.save(f, pnBuffer)

        with open('C:\\Users\\yy3\\qudi\\Data\\2024\\06\\2024-06-11\\card%d_pnBuffer%d_uint.npy' % (self.cardNo, segidx), 'wb+') as f:
            np.save(f, pnBuffer_uint)
    ...
    def set32(self, register, value):
        print('Card.set32!!!!', register, value)
        spcm_dwSetParam_i32(self.handle, register, int32(value))

    def set64(self, register, value):
        print('Card.set64!!!!', register, value)
        spcm_dwSetParam_i64(self.handle, register, int64(value))

    def get32(self, register):
        print('Card.get32!!!!', register)
        res = int32(0)
        spcm_dwGetParam_i32(self.handle, register, byref(res))
        return res.value

    def get64(self, register):
        print('Card.get64!!!!', register)
        res = int64(0)
        spcm_dwGetParam_i64(self.handle, register, byref(res))
        return res.value
