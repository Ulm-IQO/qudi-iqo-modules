...
class DataBufferCommands:
    ...
    def get_status(self):
        status = int32()
        spcm_dwGetParam_i32(self._card, SPC_M2STATUS, byref(status))
        return status.value

    def get_avail_user_len_B(self):
        c_avail_user_len = c_int64(0)
        spcm_dwGetParam_i64(self._card, SPC_DATA_AVAIL_USER_LEN, byref(c_avail_user_len))
        return c_avail_user_len.value

    def get_avail_user_pos_B(self):
        c_avail_user_pos = c_int64(0)
        spcm_dwGetParam_i64(self._card, SPC_DATA_AVAIL_USER_POS, byref(c_avail_user_pos))
        return c_avail_user_pos.value

    def get_avail_card_len_B(self):
        c_avail_card_len_B = c_int64()
        spcm_dwGetParam_i64(self._card, SPC_DATA_AVAIL_CARD_LEN, byref(c_avail_card_len_B))
        return c_avail_card_len_B.value

    def set_avail_card_len_B(self, avail_card_len_B):
        c_avail_card_len_B = c_int32(avail_card_len_B)
        spcm_dwSetParam_i32(self._card, SPC_DATA_AVAIL_CARD_LEN, c_avail_card_len_B)
        return

    def get_trig_counter(self):
        c_trig_counter = c_int64()
        spcm_dwGetParam_i64(self._card, SPC_TRIGGERCOUNTER, byref(c_trig_counter))
        return c_trig_counter.value

    def get_bits_per_sample(self):
        c_bits_per_sample = c_int32(0)
        spcm_dwGetParam_i32(self._card, SPC_MIINST_BITSPERSAMPLE, byref(c_bits_per_sample))
...
class TsBufferCommands:
    ...
    def get_gate_len_alignment(self):
        ...
        spcm_dwGetParam_i64(self._card, SPC_GATE_LEN_ALIGNMENT, byref(c_gate_len_alignment))
        return c_gate_len_alignment.value

    def get_ts_avail_user_len_B(self):
        c_ts_avail_user_len = c_int64(0)
        spcm_dwGetParam_i64(self._card, SPC_TS_AVAIL_USER_LEN, byref(c_ts_avail_user_len))
        return c_ts_avail_user_len.value

    def get_ts_avail_user_pos_B(self):
        c_ts_avail_user_pos = c_int64(0)
        spcm_dwGetParam_i64(self._card, SPC_TS_AVAIL_USER_POS, byref(c_ts_avail_user_pos))
        return c_ts_avail_user_pos.value

    def get_ts_avail_card_len_B(self):
        c_ts_avail_card_len_B = c_int64()
        spcm_dwGetParam_i64(self._card, SPC_TS_AVAIL_CARD_LEN, byref(c_ts_avail_card_len_B))
    ...
    def set_ts_avail_card_len_B(self, ts_avail_card_len_B):
        ...
        spcm_dwSetParam_i64(self._card, SPC_TS_AVAIL_CARD_LEN, c_ts_avail_card_len_B)
        return

    def get_timestamp_command(self):
        c_ts_timestamp_command = c_int64()
        spcm_dwGetParam_i64(self._card, SPC_TIMESTAMP_CMD, byref(c_ts_timestamp_command))
