...
class NiScanningProbeInterfuse(ScanningProbeInterface):
    ...
    def move_absolute(self, position, velocity=None):
        ...
        if self.is_running:
            # todo: should we allow moves during a scan?
        ...
        self.log.debug(f"Locking module to move")
        self.module_state.lock()

        self._prepare_movement(position, velocity=velocity)

        self.__start_ao_write_timer()
        # unlocking needs to be done in ao_write_loop
        #self.log.debug("Move finished, unlocking moduel. Returning target.")
    ...
    def _start_scan(self):
        ...
        # todo: this should never be called from some other (eg. logic) thread
        # emit signal and run in hardware thread!
    ...
    def _finish_move(self):
        ...
        self.module_state.unlock()
        self.log.debug('Move done. Module unlocked.')
        ...
        try:
            with self._thread_lock:
                ...
                if not all([values.size == 0 for values in self.__write_queue.values()]):
                    self.log.debug(f'Start timer for remaining write queue: {self.__write_queue}')
                    self.__start_ao_write_timer()
                else:
                    self._finish_move()
    ...
    def __start_ao_runout_timer(self):
        self.log.debug(f"ao start runout timer in thread {QtCore.QThread.currentThread()}, module thread {self.thread()}.")
        try:
            if self.thread() is not QtCore.QThread.currentThread():
    ...
    def __stop_ao_runout_timer(self):
        self.log.debug(f"ao stop runout timer in thread {QtCore.QThread.currentThread()}, module thread {self.thread()}")
        try:
            if self.thread() is not QtCore.QThread.currentThread():
                QtCore.QMetaObject.invokeMethod(self.__ni_ao_runout_timer,
                                                'stop',
    ...
    def __deactivate_ao(self):
        with self._thread_lock:
            if not all([values.size == 0 for values in self.__write_queue.values()]):
                self.log.debug('write queue not empty, try again later')
                self.__start_ao_runout_timer()
            else:
