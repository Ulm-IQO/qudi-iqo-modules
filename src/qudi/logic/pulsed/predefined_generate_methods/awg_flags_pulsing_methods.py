...
class FlagPulseGenerator(PredefinedGeneratorBase):
    ...
    def generate_4A2_lifetime_sweep(self, name='flag_sweep', tau_start=100e-9, tau_max=10e-6, num_of_points=100,
                                      tau_log=False,
                                      length_A=3e-6, delay_A=100e-9,
                                      length_B=10e-3, delay_B=100e-9, wait_AB=2e-6,
                                      length_C=100e-9, delay_C=1e-6, wait_BC=2e-6,
                                      basic_length=100e-9, last_delay=500 * 1e-9, seq_trig_D=True):
            # Add wait time between init pulse Flag A and the last reaodut pulse Flag A. Otherwise overlapping of two pulses Flag A
            #
            # sequences sweeping tau after picosecond pulse
            # make array of tau steps
            ...
            name_wait_AB = 'wait_AB'
            wait_AB_block, wait_AB_ensemble = self._get_basic_wait_ensemble(name_wait_AB, basic_length)
            wait_AB_seq_params = self._customize_seq_para({'repetitions': int((wait_AB + delay_A - delay_B) / basic_length) - 1,
                                                           'go_to': -1,
                                                           'event_jump_to': -1,
                                                           'event_trigger': 'OFF'})
            ...
            name_wait_BC = 'wait_BC'
            wait_BC_block, wait_BC_ensemble = self._get_basic_wait_ensemble(name_wait_BC, basic_length)
            wait_BC_seq_params = self._customize_seq_para({'repetitions': int((wait_BC + delay_B - delay_C) / basic_length) - 1,
                                                           'go_to': -1,
                                                           'event_jump_to': -1,
                                                           'event_trigger': 'OFF'})
            created_blocks += wait_BC_block
            ...
            for k in k_array:
                ...
                flags_pulse_params_list.append([name_wait_BC, wait_BC_seq_params])
    ...
    def generate_4A2_test(self, name='flag_pulse', length_A=3e-6, delay_A = 500e-9,
                              length_B=10e-6, delay_B=400e-9, wait_AB=2e-6,
                              length_C=100e-9, delay_C=1e-6, wait_BC=3e-6, tau_CD=2e-6,
                              basic_length=100e-9, last_delay=500 * 1e-9, seq_trig_D=True):
        # not sweeping any elements in this test sequence. Just consequently  turning on/off flags
        # next sequences could be sweeping one of these parameters
        ...
        name_wait_AB = 'wait_AB'
        wait_AB_block, wait_AB_ensemble = self._get_basic_wait_ensemble(name_wait_AB, basic_length)
        wait_AB_seq_params = self._customize_seq_para(
            {'repetitions': int((wait_AB + delay_A - delay_B) / basic_length) - 1,
             'go_to': -1,
             'event_jump_to': -1,
             'event_trigger': 'OFF'})
        ...
        name_wait_BC = 'wait_BC'
        wait_BC_block, wait_BC_ensemble = self._get_basic_wait_ensemble(name_wait_BC, basic_length)
        wait_BC_seq_params = self._customize_seq_para(
            {'repetitions': int((wait_BC + delay_B - delay_C) / basic_length) - 1,
             'go_to': -1,
             'event_jump_to': -1,
             'event_trigger': 'OFF'})
        created_blocks += wait_BC_block
        ...
        name_tau_CD = 'tau_CD'
        tau_CD_block, tau_CD_ensemble = self._get_basic_wait_ensemble(name_tau_CD, basic_length)
        tau_CD_seq_params = self._customize_seq_para(
            {'repetitions': int(tau_CD / basic_length) - 1,
             'go_to': -1,
             'event_jump_to': -1,
             'event_trigger': 'OFF'})
        created_blocks += tau_CD_block
        created_ensembles += tau_CD_ensemble
        ...
        name_last_delay = 'last_delay'
        last_delay_block, last_delay_ensemble = self._get_basic_wait_ensemble(name_last_delay, last_delay)
        last_delay_seq_params = self._customize_seq_para({'repetitions': int((last_delay) / basic_length) - 1,
                                                          'go_to': 1,
                                                          'event_jump_to': 1,
                                                          'event_trigger': 'OFF'})
        created_blocks += last_delay_block
        ...
        flags_pulse_params_list.append([name_tau_CD, tau_CD_seq_params])
    ...
    def generate_all_laser_on(self, name='all_laser_on'):
        # Create empty lists
        created_blocks = list()
        created_ensembles = list()
        created_sequences = list()
        ...
        name_all_on = 'all_on'
        all_on_block, all_on_ensemble = self._get_basic_wait_ensemble(name_all_on, 10e-6)
        all_on_seq_params = self._customize_seq_para({'repetitions': -1,
                                                      'go_to': -1,
                                                      'event_jump_to': -1,
                                                      'event_trigger': 'OFF',
                                                      'flag_high': ['A', 'B', 'C']})
        created_blocks += all_on_block
        created_ensembles += all_on_ensemble

        flags_pulse_params_list.append([name_all_on, all_on_seq_params])
        # give to PulseSequence the element_list containign serquence names and params. WHere are the correct ensembles and their names with params?
        flags_test_seq = PulseSequence(name=name, ensemble_list=flags_pulse_params_list, rotating_frame=True)
    ...
    def generate_lifetime_pulses(self, name='lifetime_pulses'):
        # Create empty lists
        created_blocks = list()
        created_ensembles = list()
        created_sequences = list()
        ...
        name_pulse = 'pulse'
        pulse_block, pulse_ensemble = self._get_basic_wait_ensemble(name_pulse, 100e-9)
        pulse_seq_params = self._customize_seq_para({'repetitions': 0,
                                                      'go_to': -1,
                                                      'event_jump_to': -1,
                                                      'event_trigger': 'OFF',
                                                      'flag_high': ['C', 'D']})
        created_blocks += pulse_block
        created_ensembles += pulse_ensemble

        name_wait = 'wait'
        wait_block, wait_ensemble = self._get_basic_wait_ensemble(name_wait, basic_length=100e-9)
        wait_seq_params = self._customize_seq_para(
            {'repetitions': 0,
             'go_to': 1,
             'event_jump_to': -1,
             'event_trigger': 'OFF'})
        ...
        # Composing the sequence
        flags_pulse_params_list.append([name_pulse, pulse_seq_params])
        flags_pulse_params_list.append([name_wait, wait_seq_params])
