...
class WavemeterLogic(LogicBase):
    ...
    def on_activate(self):
        ...
        #n = len(streamer._active_switch_channels)
        #buffer_size = 1048576
        #self._data_buffer = np.zeros(buffer_size, dtype=np.float64)
        #self._times_buffer = np.zeros(buffer_size, dtype=np.float64)
    ...
    def _counts_and_wavelength(self, new_count_data=None, new_count_timings=None):
        """
                ...
                Note: new_count_timing is currently unused
                """
        with self.threadlock:
            if self.module_state() == 'locked':
                ...
                if self._stop_requested:
                    ...
                    # terminate the hardware streaming wavelength
                ...
                #TODO Assure that the timing below is synchronized at an acceptable level (eventually use new_count_timings)

                # Determine samples to read according to new_count_data size (!one channel only!)
                self.start_time = new_count_data
                ...
                _data_wavelength, _timings = self._streamer().read_data(samples_per_channel=n)
                #self._streamer().read_data_into_buffer(data_buffer=self._data_buffer, samples_per_channel=n, timestamp_buffer=self._times_buffer)
                _data_wavelength = netobtain(_data_wavelength)
                _timings = netobtain(_timings)
                #TODO when without return value, will this cause problems with remote connection?

                #TODO buffers as an option to save the raw wavemeter values
                #Here the interpolation is performed to match the counts onto wavelength
                if n == 1:
                    #wavemeter_data, new_timings = np.ones(samples_to_read_counts)*self._data_buffer[:n], np.ones(samples_to_read_counts)*self._times_buffer[:n]
                    wavemeter_data, new_timings = np.ones(samples_to_read_counts) * _data_wavelength, np.ones(
                        samples_to_read_counts) * _timings
                else:
                    #arr_interp = interpolate.interp1d(self._times_buffer[:n], self._data_buffer[:n])
                    arr_interp = interpolate.interp1d(_timings, _data_wavelength)
                    #new_timings = np.linspace(self._times_buffer[0],
                    #                          self._times_buffer[n-1],
                    #                          samples_to_read_counts) if samples_to_read_counts > 1 else np.ones(1)*np.mean(self._times_buffer[:n])
                    new_timings = np.linspace(_timings[0],
                                              _timings[-1],
                                              samples_to_read_counts) if samples_to_read_counts > 1 else np.ones(1)*np.mean(_timings)
    ...
    @QtCore.Slot()
    def stop_scanning_through_time_series(self) -> None:
        if self.module_state() == 'locked':
            self._stop_requested = True
            time.sleep(0.5)
    ...
    def display_current_wavelength(self) -> None:
        #TODO freq or wavelength[vac or air]
    ...
    def start_displaying_current_wavelength(self) -> None:
        self._streamer().start_stream()
    ...
    def _save_data(self, postfix: Optional[str] = None, root_dir: Optional[str] = None) -> str:
        ...
        header = ['Timings (s)', 'Flourescence (counts/s)', 'Wavelength (m)', 'Frequency (Hz)', 'HistogramX', 'HistogramY', 'HistogramEnvelope']
