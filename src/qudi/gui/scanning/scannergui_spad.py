...
from qudi.logic.spad_optimize_logic import SpadOptimizeLogic
...
class ScannerGui(GuiBase):
    ...
    # declare connectors
    _scanning_logic = Connector(name="scanning_logic", interface=SpadProbeLogic)
    _data_logic = Connector(name="data_logic", interface=ScanningDataLogic)
    _optimize_logic = Connector(name="optimize_logic", interface=SpadOptimizeLogic)
    ...
    sigOptimizerSettingsChanged = QtCore.Signal(
        str, tuple, tuple, dict, dict, dict, dict, dict
    )
    ...
    def on_activate(self):
        ...
        self._mw.setWindowTitle("qudi: spad_scanner")
        ...
        self.sigFrequencyChanged.connect(
            scan_logic.set_scan_frequency, QtCore.Qt.QueuedConnection
        )
        self.sigBackFrequencyChanged.connect(
            scan_logic.set_back_scan_frequency, QtCore.Qt.QueuedConnection
        )
        self.sigUseBackScanSettings.connect(
            scan_logic.set_use_back_scan_settings, QtCore.Qt.QueuedConnection
        )
    ...
    def _init_scanner_settings(self):
        ...
        self._ssd = ScannerSettingDialog(
            scan_logic.scanner_axes.values(), scan_logic.scanner_constraints
        )

        self._ssd.settings_widget.configure_backward_scan_checkbox.setChecked(
            scan_logic.use_back_scan_settings
        )
        self._ssd.settings_widget.set_backward_settings_visibility(
            scan_logic.use_back_scan_settings
        )
        # Connect MainWindow actions
    ...
    def _init_static_dockwidgets(self):
        ...
        self.scanner_control_dockwidget.sigResolutionChanged.connect(
            scan_logic.set_scan_resolution
        )
        self.scanner_control_dockwidget.sigBackResolutionChanged.connect(
            scan_logic.set_back_scan_resolution
        )
        self.scanner_control_dockwidget.sigRangeChanged.connect(
            scan_logic.set_scan_range
        )
    ...
    def save_scan_data(self, scan_axes: Union[None, Tuple[str], Tuple[str, str]]):
        ...
        for ax in scan_axes:
            ...
            if ax in self.scan_1d_dockwidgets:
                tag = self.scan_1d_dockwidgets[
                    ax
                ].scan_widget.save_nametag_lineedit.text()
            elif ax in self.scan_2d_dockwidgets:
                tag = self.scan_2d_dockwidgets[
                    ax
                ].scan_widget.save_nametag_lineedit.text()
            else:
                tag = None
    ...
    def _add_scan_dockwidget(self, axes):
        ...
        if len(axes) == 1:
            ...
            dockwidget.scan_widget.set_plot_range(
                x_range=axes_constr[0].position.bounds
            )
            ...
            marker_bounds = (
                axes_constr[0].position.bounds,
                axes_constr[1].position.bounds,
            )
    ...
    def update_scanner_settings_from_logic(self):
        ...
        self.scanner_control_dockwidget.set_back_resolution(
            scan_logic.back_scan_resolution
        )
        ...
        self._ssd.settings_widget.configure_backward_scan = (
            scan_logic.use_back_scan_settings
        )
        self.scanner_control_dockwidget.set_backward_settings_visibility(
            scan_logic.use_back_scan_settings
    ...
    def _update_from_history(
        self, scan_data: ScanData, back_scan_data: Optional[ScanData], set_target: bool
    ):
        if set_target:
    ...
    def _update_scan_data(
        self, scan_data: ScanData, back_scan_data: Optional[ScanData]
    ):
        """
    ...
    def __get_range_from_selection_func(self, axes):
        if len(axes) == 2:
            ...
            def set_range_func(x_range, y_range):
                ...
                self.scanner_control_dockwidget.set_range(
                    {axes[0]: x_range, axes[1]: y_range}
                )
    ...
    def update_optimizer_settings_from_logic(self):
        ...
        self._osd.allowed_sequence_dimensions = (
            optimize_logic.allowed_optimizer_sequence_dimensions
        )
