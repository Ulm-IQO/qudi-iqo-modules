...
class AxesControlWidget(QtWidgets.QWidget):
    ...
    def __init__(self, *args, scanner_axes: Tuple[ScannerAxis], **kwargs):
        ...
        for index, axis in enumerate(scanner_axes, 1):
            ...
            min_spinbox.setRange(*axis.control_value.bounds)
            min_spinbox.setValue(axis.control_value.minimum)
            ...
            max_spinbox.setRange(*axis.control_value.bounds)
            max_spinbox.setValue(axis.control_value.maximum)
            ...
            init_pos = (axis.control_value.maximum - axis.control_value.minimum) / 2 + axis.control_value.minimum

            slider = DoubleSlider(QtCore.Qt.Horizontal)
            slider.setObjectName('{0}_position_doubleSlider'.format(ax_name))
            slider.setRange(*axis.control_value.bounds)
            granularity = 2 ** 31 - 1
            if axis.step.minimum > 0:
                granularity = min(granularity,
                                  round((axis.control_value.maximum - axis.control_value.minimum) / axis.step.minimum) + 1)
            ...
            pos_spinbox.setRange(*axis.control_value.bounds)
