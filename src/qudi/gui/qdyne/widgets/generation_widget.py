...
class GenerationWidget(QtWidgets.QWidget):
    ...
    def activate(self):
        ...
        self._channel_selection_comboboxes = (
            list()
        )  # List of created channel selection ComboBoxes
        self._global_param_widgets = (
            list()
        )  # List of all other created global parameter widgets
        self._create_pm_global_params()
        self.generation_parameters_updated(
            self._gui.logic().measurement_generator.generation_parameters
        )
    ...
    def connect_signals(self):
        ...
        self._gui.logic().pulsedmasterlogic().sigLoadedAssetUpdated.connect(
            self.predefined_generated
        )
        self._gui.logic().pulsedmasterlogic().sigLoadedAssetUpdated.connect(
        ...
        self._gui.logic().pulsedmasterlogic().sigSampleBlockEnsemble.connect(
            self.sampling_or_loading_busy
        )
        self._gui.logic().pulsedmasterlogic().sigLoadBlockEnsemble.connect(
            self.sampling_or_loading_busy
        )
        self._gui.logic().pulsedmasterlogic().sigLoadSequence.connect(
            self.sampling_or_loading_busy
        )
        self._gui.logic().pulsedmasterlogic().sigSampleSequence.connect(
            self.sampling_or_loading_busy
        )
    ...
    def loaded_asset_updated(self, asset_name, asset_type):
        ...
        if not asset_name:
            ...
            label.setText(
                "Currently loaded asset:  {0} ({1})".format(asset_name, asset_type)
            )
        else:
    ...
    def _create_pm_global_params(self):
        ...
        for (
            param,
            value,
        ) in self._gui.logic().measurement_generator.generation_parameters.items():
            ...
            if param.endswith("_channel") and (value is None or type(value) is str):
                ...
                widget.addItems(
                    natural_sort(
                        self._gui.logic().measurement_generator.digital_channels
                    )
                )
                widget.addItems(
                    natural_sort(
                        self._gui.logic().measurement_generator.analog_channels
                    )
                )
                ...
                self.global_param_gridLayout.addWidget(
                ...
                widget.currentIndexChanged.connect(
                    lambda: self.generation_parameters_changed()
                )
                continue
            ...
            if isinstance(value, str) or value is None:
                ...
                if "amp" in param or "volt" in param:
                    ...
                    x in param
                    for x in ("tau", "period", "time", "delay", "laser_length")
                ):
                    widget.setSuffix("s")
                widget.editingFinished.connect(self.generation_parameters_changed)
    ...
    def _create_predefined_methods(self):
        ...
        for method_name in natural_sort(
            self._gui.logic().measurement_generator.generate_methods
        ):
            ...
            gen_button.clicked.connect(
                self.__generate_predefined_slot(method_name, False)
            )
            ...
            samplo_button.clicked.connect(
                self.__generate_predefined_slot(method_name, True)
            )
            ...
            for param_index, (param_name, param) in enumerate(
                method_params[method_name].items()
            ):
                ...
                if type(param) is bool:
                    ...
                    if "amp" in param_name or "volt" in param_name:
                        ...
                        "time" in param_name
                        or "period" in param_name
                        or "tau" in param_name
                    ):
    ...
    def predefined_generated(self, asset_name):
        ...
        if (
            asset_name is None
            or not self._gui.logic().measurement_generator.status_dict["sampload_busy"]
        ):
            for button in self.samplo_buttons.values():
    ...
    def counter_settings_updated(self, settings_dict):
        ...
        if "is_gated" in settings_dict:
            if settings_dict.get("is_gated"):
                ...
                self._gui.logic().measurement_generator.set_generation_parameters(
                    {"gate_channel": ""}
                )
    ...
    def toggle_measurement_settings_editor(self, hide_editor):
        ...
        if hide_editor:
            ...
            self.ana_param_sequence_length_DoubleSpinBox.setEnabled(False)
        else:
            self.ana_param_record_length_DoubleSpinBox.setEnabled(True)
            self.ana_param_sequence_length_DoubleSpinBox.setEnabled(True)
        return
